    ...
    this.display.addEventListener('click', (e) => {
      ...
      if (!abierto) {
        ...
        requestAnimationFrame(() => {
          ...
          let top;
          if (displayRect.bottom + gap + panelH <= window.innerHeight) {
            top = displayRect.bottom + gap;
          } else if (displayRect.top - gap - panelH >= 0) {
            top = displayRect.top - gap - panelH;
          } else {
            top = Math.max(gap, (window.innerHeight - panelH) / 2);
          }

          let left = displayRect.left + displayRect.width / 2 - panelW / 2;

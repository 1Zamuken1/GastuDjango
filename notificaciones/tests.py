...
class NotificacionesViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.notificacion_leida = Notificacion.objects.create(
            usuario=self.user,
            titulo='Test Notification Read',
            tipo='DEFICIT',
            modulo='MOVIMIENTOS',
            descripcion='This is a test notification that is read',
            leida=True
        )
        
        self.notificacion_no_leida = Notificacion.objects.create(
            usuario=self.user,
            titulo='Test Notification Unread',
            tipo='EGRESO_GRANDE',
            modulo='MOVIMIENTOS',
            descripcion='This is a test notification that is not read',
            leida=False
        )

    def test_notificaciones_json_view(self):
        """Test the notificaciones_json view"""
        response = self.client.get(reverse('notificaciones:notificaciones_json'))
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data['ok'], True)
        self.assertEqual(len(data['notificaciones']), 2)
        self.assertEqual(data['total_no_leidas'], 1)
        self.assertEqual(data['total_no_leidas_modulo'], 1)

    def test_notificaciones_json_view_with_modulo_filter(self):
        """Test the notificaciones_json view with modulo filter"""
        response = self.client.get(
            reverse('notificaciones:notificaciones_json') + 
            '?modulo=MOVIMIENTOS'
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_notificaciones_json_view_with_todos_modulo(self):
        """Test the notificaciones_json view with modulo=TODOS"""
        response = self.client.get(
            reverse('notificaciones:notificaciones_json') + 
            '?modulo=TODOS'
        )
    ...
    def test_notificaciones_marcar_leidas_view_post_no_body(self):
        """Test marking notifications as read with no body (marks all)"""
        response = self.client.post(
            reverse('notificaciones:notificaciones_marcar_leidas'),
            data=json.dumps({}),
        ...
        data = response.json()
        self.assertEqual(data['ok'], True)
        self.assertEqual(data['afectadas'], 1)  # Only the unread one should be affected
        
        # Verify the notification is now marked as read
        self.notificacion_no_leida.refresh_from_db()
        self.assertTrue(self.notificacion_no_leida.leida)

    def test_notificaciones_marcar_leidas_view_post_with_ids(self):
        """Test marking specific notifications as read by IDs"""
        response = self.client.post(
            reverse('notificaciones:notificaciones_marcar_leidas'),
            data=json.dumps({'ids': [self.notificacion_no_leida.id]}),
            content_type='application/json'
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_notificaciones_marcar_leidas_view_post_with_modulo(self):
        """Test marking all notifications of a module as read"""
        response = self.client.post(
            reverse('notificaciones:notificaciones_marcar_leidas'),
            data=json.dumps({'modulo': 'MOVIMIENTOS'}),
            content_type='application/json'
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data['ok'], True)
        self.assertEqual(data['afectadas'], 1)  # Only one unread in MOVIMIENTOS

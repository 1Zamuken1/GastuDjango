...
async function actualizarGrid() {
  try {
    const res = await fetch(`${URL_RESUMEN}?tipo=INGRESO`, {
    ...
    if (data.categorias.length === 0) {
      grid.innerHTML = '<div class="grid-empty">Sin ingresos registrados este mes</div>';
      return;
    }
    ...
    grid.innerHTML = data.categorias.map(cat => `
      <div class="categoria-card"
        ...
        <div style="display:flex;align-items:center;gap:.5rem;margin:.5rem 0 .25rem;">
          ...
          <span style="font-size:.72rem;font-weight:700;color:#10b981;white-space:nowrap;min-width:2.5rem;text-align:right;">${cat.porcentaje}%</span>
        </div>
  ...
  debounceTimer = setTimeout(async () => {
    ...
    try {
      const res = await fetch(
        `${URL_BUSCAR}?q=${encodeURIComponent(q)}&tipo=INGRESO`,
        { headers: { 'X-Requested-With': 'XMLHttpRequest' } }
      );
      const data = await res.json();
      if (!data.ok) return;
      ...
      if (idsConResultados.size > 0) {
        document.querySelectorAll('.categoria-card').forEach(card => {
          ...
          card.style.outline = tiene ? '2px solid #10b981' : '';
        });
      }
    } catch (err) {
...
function _inicializarDatepickers() {
  if (!dpDesde) {
    ...
    dpDesde = new MiniDatepicker(inputDesde);
    dpHasta = new MiniDatepicker(inputHasta);
...
(function() {
  ...
  if (inputFecha) {
    const dp = new MiniDatepicker(inputFecha);
